...
async function deploySolana(
...
): Promise<DeploymentAddresses['solana']> {
  ...
  if (keypairPath && fs.existsSync(keypairPath)) {
    ...
    console.log(
      '⚠️  Generating new keypair for deployment (save this for future use!)'
    );
...
async function main() {
  ...
  if (!evmNetwork && !solanaNetwork) {
    console.log(
      'Usage: npx ts-node scripts/unified/deploy-all.ts [--evm=network] [--solana=network] [--keypair=path] [--output=path]'
    );
    console.log(
