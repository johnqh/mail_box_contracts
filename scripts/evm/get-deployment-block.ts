...
async function main() {
  ...
  if (!address) {
    console.error(
      'Usage: CONTRACT_ADDRESS=<address> npx hardhat run scripts/evm/get-deployment-block.ts --network <network>'
    );
    console.error(
      'Example: CONTRACT_ADDRESS=0x13fC7Fe676E4FaaE8F4D910d8Ed7fbD3FebDbe88 npx hardhat run scripts/evm/get-deployment-block.ts --network sepolia'
    );
    process.exit(1);
  }
