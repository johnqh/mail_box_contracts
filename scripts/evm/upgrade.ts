...
async function main() {
  ...
  if (!proxyAddress) {
    ...
    console.error(
      'Please provide PROXY_ADDRESS environment variable or ensure deployment file exists.'
    );
    console.error(
      'Example: PROXY_ADDRESS=0x... npx hardhat run scripts/evm/upgrade.ts --network sepolia'
    );
    process.exit(1);
  }
  ...
  try {
    ...
    console.log(
      '   - Send Fee:',
      formatUnits(sendFeeAfter, 6),
      'USDC',
      sendFeeAfter === sendFeeBefore ? '✅' : '❌'
    );
    console.log(
      '   - Owner:',
      ownerAfter,
    ...
    if (
      ...
      console.error(
        '❌ State verification failed! Some values were not preserved.'
      );
      process.exit(1);
    }
