...
describe('Mailer', function () {
  beforeEach(async function () {
    ...
    await mockUSDC.write.approve([mailer.address, parseUnits('10', 6)], {
  ...
  describe('Contract setup', function () {
    it('Should set USDC token address correctly', async function () {
      ...
      expect((await mailer.read.usdcToken()).toLowerCase()).to.equal(
    ...
    it('Should set owner as deployer', async function () {
      ...
      expect((await mailer.read.owner()).toLowerCase()).to.equal(
  ...
  describe('send function with revenue sharing (priority mode)', function () {
    it('Should emit MailSent event when USDC transfer succeeds', async function () {
      ...
      const hash = await mailer.write.send(
        [
      ...
      expect(event.args.from.toLowerCase()).to.equal(
        addr1.account.address.toLowerCase()
      );
      expect(event.args.to.toLowerCase()).to.equal(
  ...
  describe('sendPrepared function with revenue sharing (priority mode)', function () {
    it('Should emit PreparedMailSent event when USDC transfer succeeds', async function () {
      const { mailer, addr1, addr2, publicClient } = this;
      const hash = await mailer.write.sendPrepared(
        [addr2.account.address, 'mail-123', addr1.account.address, true, false],
        { account: addr1.account }
      );
    ...
    it('Should emit event with feePaid=false when USDC transfer fails (insufficient balance)', async function () {
      ...
      const ownerBefore = await mailer.read.getOwnerClaimable();
      const hash = await mailer.write.sendPrepared(
        [addr1.account.address, 'mail-456', addr2.account.address, true, false],
        { account: addr2.account }
      );
      await publicClient.waitForTransactionReceipt({ hash });
    ...
    it('Should emit event with feePaid=false when USDC transfer fails (insufficient allowance)', async function () {
      ...
      const ownerBefore = await mailer.read.getOwnerClaimable();
      const hash = await mailer.write.sendPrepared(
        [addr1.account.address, 'mail-789', addr2.account.address, true, false],
        { account: addr2.account }
      );
      await publicClient.waitForTransactionReceipt({ hash });
    ...
    it('Should transfer correct USDC amount to contract', async function () {
      ...
      await mailer.write.sendPrepared(
        [addr2.account.address, 'mail-999', addr1.account.address, true, false],
        { account: addr1.account }
      );
  ...
  describe('Fee management', function () {
    ...
    describe('setFee function', function () {
      it('Should allow owner to update fee', async function () {
        ...
        const hash = await mailer.write.setFee([newFee], {
          account: owner.account,
        });
        await publicClient.waitForTransactionReceipt({ hash });
      ...
      it('Should allow setting fee to zero', async function () {
        const { mailer, owner, publicClient } = this;
        const hash = await mailer.write.setFee([0n], {
          account: owner.account,
        });
        await publicClient.waitForTransactionReceipt({ hash });
      ...
      it('Should allow setting very high fee', async function () {
        ...
        const hash = await mailer.write.setFee([highFee], {
          account: owner.account,
        });
        await publicClient.waitForTransactionReceipt({ hash });
      ...
      it('Should emit correct event with old and new fee values', async function () {
        ...
        const hash = await mailer.write.setFee([250000n], {
          account: owner.account,
        });
        await publicClient.waitForTransactionReceipt({ hash });
    ...
    describe('Fee functionality integration', function () {
      ...
      it('Should emit feePaid=false when user has insufficient balance for new fee', async function () {
        ...
        // Set a very high fee
        await mailer.write.setFee([parseUnits('20', 6)], {
          account: owner.account,
        }); // 20 USDC
  ...
  describe('send function (standard mode - no revenue share)', function () {
    ...
    it('Should emit feePaid=false when sender has no USDC balance', async function () {
      ...
      await mockUSDC.write.transfer(
        [owner.account.address, parseUnits('10', 6)],
        { account: addr2.account }
      );
      const ownerBefore = await mailer.read.getOwnerClaimable();
    ...
    it('Should emit feePaid=false when sender has no USDC allowance', async function () {
      const { mailer, mockUSDC, addr1, addr2, publicClient } = this;
      // Revoke allowance
      await mockUSDC.write.approve([mailer.address, 0n], {
        account: addr2.account,
      });
    ...
    it('Should transfer 10% of sendFee to contract for owner', async function () {
      ...
      expect(finalOwnerClaimable - initialOwnerClaimable).to.equal(
        expectedOwnerFee
      );
    });
    ...
    it('Should work with empty strings', async function () {
      const { mailer, addr1, addr2, publicClient } = this;
      const hash = await mailer.write.send(
        [addr1.account.address, '', '', addr2.account.address, false, false],
        { account: addr2.account }
      );
  ...
  describe('sendThroughWebhook function with revenue sharing (priority mode)', function () {
    it('Should emit WebhookMailSent event when USDC transfer succeeds', async function () {
      const { mailer, addr1, addr2, publicClient } = this;
      const hash = await mailer.write.sendThroughWebhook(
        [
          addr2.account.address,
          'webhook-123',
          addr1.account.address,
    ...
    it('Should transfer correct USDC amount to contract', async function () {
      ...
      await mailer.write.sendThroughWebhook(
        [
          addr2.account.address,
          'webhook-999',
          addr1.account.address,
    ...
    it('Should record 90% for recipient and 10% for owner with revenue share', async function () {
      ...
      const [amount, ,] = await mailer.read.getRecipientClaimable([
        addr2.account.address,
      ]);
  ...
  describe('Revenue Sharing System', function () {
    beforeEach(async function () {
      ...
      await mockUSDC.write.approve([mailer.address, parseUnits('100', 6)], {
        account: addr1.account,
      });
    });
    ...
    describe('Recipient Claims', function () {
      ...
      it('Should allow recipient to claim their share', async function () {
        ...
        const hash = await mailer.write.claimRecipientShare([], {
          account: addr2.account,
        });
        await publicClient.waitForTransactionReceipt({ hash });
        ...
        expect(logs.length).to.be.greaterThan(0);
        const event = logs[logs.length - 1];
        expect(event.args.recipient.toLowerCase()).to.equal(
          addr2.account.address.toLowerCase()
        );
        ...
        const [newAmount, ,] = await mailer.read.getRecipientClaimable([
          addr2.account.address,
        ]);
      ...
      it('Should handle expired claims correctly', async function () {
        ...
        const [expiredAmount, ,] = await mailer.read.getRecipientClaimable([
          addr2.account.address,
        ]);
        const initialOwnerClaimable = await mailer.read.getOwnerClaimable();
        ...
        const hash = await mailer.write.claimExpiredShares(
          [addr2.account.address],
          { account: owner.account }
        );
        await publicClient.waitForTransactionReceipt({ hash });
        ...
        const finalOwnerClaimable = await mailer.read.getOwnerClaimable();
        expect(finalOwnerClaimable).to.equal(
          initialOwnerClaimable + expiredAmount
        );
      });
    });
    ...
    describe('Owner Claims', function () {
      ...
      it('Should allow owner to claim their share', async function () {
        ...
        const hash = await mailer.write.claimOwnerShare([], {
          account: owner.account,
        });
        await publicClient.waitForTransactionReceipt({ hash });
    ...
    describe('Expired Share Management', function () {
      ...
      it('Should revert if claim period not expired', async function () {
        ...
        await expect(
          mailer.write.claimExpiredShares([addr2.account.address], {
            account: owner.account,
          })
    ...
    describe('View Functions', function () {
      it('Should return correct recipient claimable info', async function () {
        ...
        let [amount, expiresAt, isExpired] =
          await mailer.read.getRecipientClaimable([addr2.account.address]);
    ...
    describe('Delegation functionality', function () {
      ...
      describe('delegateTo function', function () {
        it('Should allow delegation and charge fee', async function () {
          ...
          const hash = await mailer.write.delegateTo([addr2.account.address], {
            account: addr1.account,
          });
          await publicClient.waitForTransactionReceipt({ hash });
          ...
          expect(logs.length).to.be.greaterThan(0);
          const event = logs[logs.length - 1];
          expect(event.args.delegator.toLowerCase()).to.equal(
            addr1.account.address.toLowerCase()
          );
          expect(event.args.delegate.toLowerCase()).to.equal(
            addr2.account.address.toLowerCase()
          );
        ...
        it('Should allow clearing delegation without fee', async function () {
          ...
          await mailer.write.delegateTo([addr2.account.address], {
            account: addr1.account,
          });
          ...
          const hash = await mailer.write.delegateTo([zeroAddress], {
            account: addr1.account,
          });
          await publicClient.waitForTransactionReceipt({ hash });
        ...
        it('Should revert if USDC transfer fails', async function () {
          ...
          await expect(
            mailer.write.delegateTo([addr2.account.address], {
              account: addr1.account,
            })
        ...
        it('Should credit owner claimable balance and allow owner to withdraw delegation fees', async function () {
          ...
          const contractBalanceBefore = await mockUSDC.read.balanceOf([
            mailer.address,
          ]);
          const delegationFee = await mailer.read.delegationFee();
          ...
          const contractBalanceAfterDelegation = await mockUSDC.read.balanceOf([
            mailer.address,
          ]);
          expect(
            contractBalanceAfterDelegation - contractBalanceBefore
          ).to.equal(delegationFee);

          const claimHash = await mailer.write.claimOwnerShare([], {
            account: owner.account,
          });
          ...
          const contractBalanceAfterClaim = await mockUSDC.read.balanceOf([
            mailer.address,
          ]);
      ...
      describe('rejectDelegation function', function () {
        it('Should emit DelegationSet event with zero address', async function () {
          const { mailer, addr1, addr2, publicClient } = this;
          const hash = await mailer.write.rejectDelegation(
            [addr1.account.address],
            { account: addr2.account }
          );
          await publicClient.waitForTransactionReceipt({ hash });
      ...
      describe('Delegation fee management', function () {
        it('Should allow owner to update delegation fee', async function () {
          ...
          const hash = await mailer.write.setDelegationFee([newFee], {
            account: owner.account,
          });
          await publicClient.waitForTransactionReceipt({ hash });
        ...
        it('Should revert when non-owner tries to update delegation fee', async function () {
          const { mailer, addr1 } = this;
          await expect(
            mailer.write.setDelegationFee([parseUnits('5', 6)], {
              account: addr1.account,
            })
          ).to.be.rejectedWith('OwnableUnauthorizedAccount');
        });
    ...
    describe('Custom Fee Percentage functionality', function () {
      ...
      describe('setCustomFeePercentage function', function () {
        it('Should allow owner to set custom fee percentage', async function () {
          ...
          const hash = await mailer.write.setCustomFeePercentage(
            [addr1.account.address, 50],
            { account: owner.account }
          );
          await publicClient.waitForTransactionReceipt({ hash });
          ...
          expect(logs.length).to.be.greaterThan(0);
          const event = logs[logs.length - 1];
          expect(event.args.account.toLowerCase()).to.equal(
            addr1.account.address.toLowerCase()
          );
          ...
          expect(
            await mailer.read.customFeeDiscount([addr1.account.address])
          ).to.equal(50);
        });
        ...
        it('Should revert when non-owner tries to set percentage', async function () {
          const { mailer, addr1, addr2 } = this;
          await expect(
            mailer.write.setCustomFeePercentage([addr2.account.address, 50], {
              account: addr1.account,
            })
          ).to.be.rejectedWith('OwnableUnauthorizedAccount');
        });
        ...
        it('Should revert when percentage > 100', async function () {
          ...
          await expect(
            mailer.write.setCustomFeePercentage([addr1.account.address, 101], {
              account: owner.account,
            })
        ...
        it('Should revert when address is zero', async function () {
          const { mailer, owner } = this;
          await expect(
            mailer.write.setCustomFeePercentage([zeroAddress, 50], {
              account: owner.account,
            })
      ...
      describe('clearCustomFeePercentage function', function () {
        beforeEach(async function () {
          ...
          await mailer.write.setCustomFeePercentage(
            [addr1.account.address, 75],
            { account: owner.account }
          );
        });
        ...
        it('Should allow owner to clear custom fee percentage', async function () {
          const { mailer, owner, addr1, publicClient } = this;
          const hash = await mailer.write.clearCustomFeePercentage(
            [addr1.account.address],
            { account: owner.account }
          );
          await publicClient.waitForTransactionReceipt({ hash });
        ...
        it('Should revert when non-owner tries to clear percentage', async function () {
          const { mailer, addr1 } = this;
          await expect(
            mailer.write.clearCustomFeePercentage([addr1.account.address], {
              account: addr1.account,
            })
          ).to.be.rejectedWith('OwnableUnauthorizedAccount');
        });
        ...
        it('Should revert when address is zero', async function () {
          const { mailer, owner } = this;
          await expect(
            mailer.write.clearCustomFeePercentage([zeroAddress], {
              account: owner.account,
            })
          ).to.be.rejectedWith('InvalidAddress');
        });
      ...
      describe('getCustomFeePercentage function', function () {
        it('Should return 100 for address without custom percentage', async function () {
          ...
          expect(
            await mailer.read.getCustomFeePercentage([addr1.account.address])
          ).to.equal(100);
        });
      ...
      describe('send function with custom fee percentage', function () {
        ...
        it('Should not charge fee or record shares when percentage is 0', async function () {
          ...
          const [recipientAmount, ,] = await mailer.read.getRecipientClaimable([
            addr2.account.address,
          ]);
      ...
      describe('sendToEmailAddress with custom fee percentage', function () {
        it('Should apply custom percentage to email sends', async function () {
          ...
          await mailer.write.sendToEmailAddress(
            ['test@example.com', 'Subject', 'Body', addr1.account.address],
            { account: addr1.account }
          );
          const finalBalance = await mockUSDC.read.balanceOf([mailer.address]);
      ...
      describe('sendPreparedToEmailAddress with custom fee percentage', function () {
        it('Should apply custom percentage to prepared email sends', async function () {
          ...
          await mailer.write.sendPreparedToEmailAddress(
            ['test@example.com', 'mail-789', addr1.account.address],
            { account: addr1.account }
          );
          const finalBalance = await mockUSDC.read.balanceOf([mailer.address]);
      ...
      describe('Prevent changes when paused', function () {
        it('Should revert setCustomFeePercentage when paused', async function () {
          ...
          await expect(
            mailer.write.setCustomFeePercentage([addr1.account.address, 50], {
              account: owner.account,
            })
    ...
    describe('Pause functionality', function () {
      ...
      it('Should distribute owner claimable funds when pausing', async function () {
        ...
        await mockUSDC.write.approve([mailer.address, parseUnits('1', 6)], {
          account: addr1.account,
        });
        await mailer.write.send(
          [
        ...
        const contractBalanceAfter = await mockUSDC.read.balanceOf([
          mailer.address,
        ]);
      ...
      it('Should allow anyone to distribute claimable funds when paused', async function () {
        ...
        const [testOwner, testAddr1, testAddr2, testAddr3] =
        ...
        const testMailerAddress = getAddress(
          await testMailerProxy.getAddress()
        );
        const testMailer = await hre.viem.getContractAt(
          'Mailer',
          testMailerAddress
        );
        ...
        await testMockUSDC.write.mint([
          testAddr1.account.address,
          parseUnits('1', 6),
        ]);
        await testMockUSDC.write.approve(
          [testMailer.address, parseUnits('1', 6)],
          { account: testAddr1.account }
        );

        // testAddr1 sends to testAddr2 with revenue sharing - testAddr2 gets the 90% claimable amount
        await testMailer.write.send(
          [
            testAddr2.account.address,
            'Test',
            'Test',
            testAddr1.account.address,
            true,
        ...
        const [claimableAmount] = await testMailer.read.getRecipientClaimable([
          testAddr2.account.address,
        ]);
        ...
        const [claimableAmountAfterPause] =
          await testMailer.read.getRecipientClaimable([
            testAddr2.account.address,
          ]);
        expect(claimableAmountAfterPause).to.equal(claimableAmount); // Should be unchanged

        const recipientBalanceBefore = await testMockUSDC.read.balanceOf([
          testAddr2.account.address,
        ]);

        // Anyone can distribute claimable funds when paused
        const hash = await testMailer.write.distributeClaimableFunds(
          [testAddr2.account.address],
          { account: testAddr3.account }
        );
        ...
        const event = logs[logs.length - 1];
        expect(event.args.recipient.toLowerCase()).to.equal(
          testAddr2.account.address.toLowerCase()
        );
        expect(event.args.amount).to.equal(claimableAmount);

        const recipientBalanceAfter = await testMockUSDC.read.balanceOf([
          testAddr2.account.address,
        ]);
        expect(recipientBalanceAfter).to.equal(
          recipientBalanceBefore + claimableAmount
        );
      ...
      it('Should revert distribute when contract not paused', async function () {
        const { mailer, addr1, addr2 } = this;
        await expect(
          mailer.write.distributeClaimableFunds([addr2.account.address], {
            account: addr1.account,
          })
        ).to.be.rejectedWith('ContractNotPaused');
      });
      ...
      it('Should allow emergency unpause by owner', async function () {
        ...
        const hash = await mailer.write.emergencyUnpause([], {
          account: owner.account,
        });
        await publicClient.waitForTransactionReceipt({ hash });
      ...
      it('Should prevent fee changes when paused', async function () {
        ...
        await expect(
          mailer.write.setFee([parseUnits('0.2', 6)], {
            account: owner.account,
          })
        ).to.be.rejectedWith('ContractIsPaused');

        await expect(
          mailer.write.setDelegationFee([parseUnits('20', 6)], {
            account: owner.account,
          })
        ).to.be.rejectedWith('ContractIsPaused');
      });
    ...
    describe('Permission System', function () {
      ...
      describe('setPermission', function () {
        it('Should allow wallet to grant permission to a contract', async function () {
          ...
          const hash = await mailer.write.setPermission([contractAddr], {
            account: addr1.account,
          });
          await publicClient.waitForTransactionReceipt({ hash });
          ...
          expect(
            await mailer.read.permissions([contractAddr, addr1.account.address])
          ).to.be.true;
        });
        ...
        it('Should revert when trying to grant permission to zero address', async function () {
          const { mailer, addr1 } = this;
          await expect(
            mailer.write.setPermission([zeroAddress], {
              account: addr1.account,
            })
          ).to.be.rejectedWith('InvalidAddress');
        });
        ...
        it('Should allow same wallet to grant permission to multiple contracts', async function () {
          ...
          await mailer.write.setPermission([contract1], {
            account: addr1.account,
          });
          await mailer.write.setPermission([contract2], {
            account: addr1.account,
          });

          expect(
            await mailer.read.permissions([contract1, addr1.account.address])
          ).to.be.true;
          expect(
            await mailer.read.permissions([contract2, addr1.account.address])
          ).to.be.true;
        });
        ...
        it('Should allow multiple wallets to grant permission to same contract', async function () {
          ...
          await mailer.write.setPermission([contractAddr], {
            account: addr1.account,
          });
          await mailer.write.setPermission([contractAddr], {
            account: owner.account,
          ...
          expect(
            await mailer.read.permissions([contractAddr, owner.account.address])
          ).to.be.true;
        });
        ...
        it('Should revert when paused', async function () {
          ...
          await expect(
            mailer.write.setPermission([contractAddr], {
              account: addr1.account,
            })
          ).to.be.rejectedWith('ContractIsPaused');
        });
      ...
      describe('removePermission', function () {
        ...
        it('Should allow wallet to revoke permission from a contract', async function () {
          const { mailer, addr1, publicClient } = this;
          const hash = await mailer.write.removePermission([contractAddr], {
            account: addr1.account,
          });
          await publicClient.waitForTransactionReceipt({ hash });
        ...
        it('Should allow revoking permission even when not paused', async function () {
          ...
          await mailer.write.removePermission([contractAddr], {
            account: addr1.account,
          });
    ...
    describe('Email Sending Functions', function () {
      ...
      describe('sendToEmailAddress', function () {
        it('Should emit MailSentToEmail event when USDC transfer succeeds', async function () {
          const { mailer, addr1, publicClient } = this;
          const hash = await mailer.write.sendToEmailAddress(
            [
              'test@example.com',
              'Test Subject',
              'Test Body',
        ...
        it('Should emit feePaid=false when USDC transfer fails (insufficient balance)', async function () {
          ...
          const hash = await mailer.write.sendToEmailAddress(
            ['test@example.com', 'Subject', 'Body', addr2.account.address],
            { account: addr2.account }
          );
          await publicClient.waitForTransactionReceipt({ hash });
        ...
        it('Should work with empty strings', async function () {
          const { mailer, addr1, publicClient } = this;
          const hash = await mailer.write.sendToEmailAddress(
            ['test@example.com', '', '', addr1.account.address],
            { account: addr1.account }
          );
        ...
        it('Should work with long strings', async function () {
          ...
          const hash = await mailer.write.sendToEmailAddress(
            ['test@example.com', longSubject, longBody, addr1.account.address],
            { account: addr1.account }
          );
        ...
        it('Should work with different email formats', async function () {
          ...
          for (const email of emails) {
            const hash = await mailer.write.sendToEmailAddress(
              [email, 'Subject', 'Body', addr1.account.address],
              { account: addr1.account }
            );
            await publicClient.waitForTransactionReceipt({ hash });
          }
        ...
        it('Should revert when paused', async function () {
          ...
          await expect(
            mailer.write.sendToEmailAddress(
              ['test@example.com', 'Subject', 'Body', addr1.account.address],
              { account: addr1.account }
            )
          ).to.be.rejectedWith('ContractIsPaused');
      ...
      describe('sendPreparedToEmailAddress', function () {
        it('Should emit PreparedMailSentToEmail event when USDC transfer succeeds', async function () {
          const { mailer, addr1, publicClient } = this;
          const hash = await mailer.write.sendPreparedToEmailAddress(
            ['test@example.com', 'mail-123', addr1.account.address],
            { account: addr1.account }
          );
        ...
        it('Should transfer correct USDC amount to contract', async function () {
          ...
          await mailer.write.sendPreparedToEmailAddress(
            ['test@example.com', 'mail-456', addr1.account.address],
            { account: addr1.account }
          );
        ...
        it('Should emit feePaid=false when USDC transfer fails (insufficient balance)', async function () {
          ...
          const hash = await mailer.write.sendPreparedToEmailAddress(
            ['test@example.com', 'mail-789', addr2.account.address],
            { account: addr2.account }
          );
          await publicClient.waitForTransactionReceipt({ hash });
        ...
        it('Should emit feePaid=false when USDC transfer fails (insufficient allowance)', async function () {
          ...
          const ownerBefore = await mailer.read.getOwnerClaimable();
          const hash = await mailer.write.sendPreparedToEmailAddress(
            ['test@example.com', 'mail-abc', addr2.account.address],
            { account: addr2.account }
          );
          await publicClient.waitForTransactionReceipt({ hash });
        ...
        it('Should work with long mailId', async function () {
          ...
          const hash = await mailer.write.sendPreparedToEmailAddress(
            ['test@example.com', longMailId, addr1.account.address],
            { account: addr1.account }
          );
        ...
        it('Should handle special characters in mailId', async function () {
          ...
          const hash = await mailer.write.sendPreparedToEmailAddress(
            ['test@example.com', specialMailId, addr1.account.address],
            { account: addr1.account }
          );
        ...
        it('Should work with different email formats', async function () {
          ...
          for (const email of emails) {
            const hash = await mailer.write.sendPreparedToEmailAddress(
              [email, `mail-${email}`, addr1.account.address],
              { account: addr1.account }
            );
            await publicClient.waitForTransactionReceipt({ hash });
          }
        ...
        it('Should revert when paused', async function () {
          ...
          await expect(
            mailer.write.sendPreparedToEmailAddress(
              ['test@example.com', 'mail-123', addr1.account.address],
              { account: addr1.account }
            )
          ).to.be.rejectedWith('ContractIsPaused');
