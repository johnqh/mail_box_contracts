...
describe('Mailer Upgrades (UUPS)', function () {
  ...
  describe('Proxy deployment', function () {
    it('Should deploy as UUPS proxy', async function () {
      ...
      const implementationAddress =
        await upgrades.erc1967.getImplementationAddress(mailer.address);
  ...
  describe('Upgrade functionality', function () {
    ...
    it('Should preserve state after upgrade', async function () {
      ...
      const claimableBefore = await mailer.read.getRecipientClaimable([
        addr2.account.address,
      ]);
      ...
      const upgradedMailer = await hre.viem.getContractAt(
        'Mailer',
        mailer.address
      );
      ...
      expect((await upgradedMailer.read.usdcToken()).toLowerCase()).to.equal(
        usdcBefore.toLowerCase()
      );
      expect((await upgradedMailer.read.owner()).toLowerCase()).to.equal(
        ownerBefore.toLowerCase()
      );

      const claimableAfter = await upgradedMailer.read.getRecipientClaimable([
        addr2.account.address,
      ]);
    ...
    it('Should continue to work after upgrade', async function () {
      const { mailer, addr1, addr2, mockUSDC, Mailer, upgrades, publicClient } =
        this;
      ...
      await mockUSDC.write.approve(
        [upgradedMailer.address, parseUnits('10', 6)],
        { account: addr1.account }
      );
  ...
  describe('Storage layout validation', function () {
    it('Should maintain storage layout after upgrade', async function () {
      ...
      await mailer.write.setFee([parseUnits('0.2', 6)], {
        account: owner.account,
      });
      await mailer.write.setDelegationFee([parseUnits('20', 6)], {
        account: owner.account,
      });
      ...
      const recipientClaim = await mailer.read.getRecipientClaimable([
        addr2.account.address,
      ]);
      ...
      expect(await upgradedMailer.read.ownerClaimable()).to.equal(
        ownerClaimable
      );

      const recipientClaimAfter =
        await upgradedMailer.read.getRecipientClaimable([
          addr2.account.address,
        ]);
