...
it('Should initialize mailer account', async () => {
  ...
  const serviceAccount =
    await program.account.mailServiceState.fetch(mailServicePda);
  expect(serviceAccount.owner.toString()).to.equal(
...
describe('Error Handling', () => {
  it('should handle insufficient balance gracefully', async () => {
    ...
    await expect(
      unfundedClient.sendMessage('Test', 'Body', true)
  ...
  it('should handle invalid address formats', async () => {
    ...
    await expect(client.delegateTo('invalid-address')).to.be.rejectedWith(
  ...
  it('should handle network timeouts', async () => {
    ...
    await expect(client.sendMessage('Test', 'Body', true)).to.be.rejectedWith(
      'Connection timeout'
    );
  });
});
